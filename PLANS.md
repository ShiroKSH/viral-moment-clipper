...
- Personal ranker validation done: separate dataset/evaluation/registry modules, Pydantic feature validation, fold-local scaling, validation by source project, baseline-gated activation, latest publication captures, missing-retention handling, cached batch prediction, and atomic model files with reproducible JSON reports.
